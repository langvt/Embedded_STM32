...
static void CLCD_WriteI2C(CLCD_I2C_Name* LCD, uint8_t Data, uint8_t Mode)
{
	...
	I2C_Write_No_Reg(LCD->ADDRESS, Data_I2C[0] );
	I2C_Write_No_Reg(LCD->ADDRESS, Data_I2C[1] );
	I2C_Write_No_Reg(LCD->ADDRESS, Data_I2C[2] );
	I2C_Write_No_Reg(LCD->ADDRESS, Data_I2C[3] );
}


//************************** High Level Function ****************************************************************//
void CLCD_I2C_Init(CLCD_I2C_Name* LCD,  uint8_t Address, uint8_t Rows, uint8_t Colums)
{
	LCD->ADDRESS = Address;
	LCD->COLUMS = Colums;
	LCD->ROWS = Rows;
...
void CLCD_I2C_SetCursor(CLCD_I2C_Name* LCD, uint8_t Rows, uint8_t Colums)
{	
		uint8_t Rows_Offsets[] = { 0x00, 0x40, 0x14, 0x54 };

		CLCD_WriteI2C(LCD,LCD_SETDDRAMADDR | (Colums + Rows_Offsets[Rows]), CLCD_COMMAND);
}
